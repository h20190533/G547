...
#define BOMS_RESETREQTYPE           0x21
#define BOMS_GET_MAX_LUN              0xFE
#define BOMS_GETMAXLUNREQTYPE     0xA1
...
{	struct command_status_wrapper *csw;
	...
	if(r<0)
		printk(KERN_INFO "ERROR IN RECIVING STATUS MESG %d",r);
	if (size != 13) {
	...
	uint8_t *cdb, uint8_t direction, int data_length, uint32_t *ret_tag)
{
	...
	*ret_tag = tag;
	...
	memcpy(cbw->CBWCB, cdb, cdb_len);
        ...
        if(r!=0)
		    printk(KERN_INFO "Failed command transfer %d",r);
	    else 	
	    	printk(KERN_INFO "read capacity command sent successfully");
...
static int test_mass_storage (struct usb_device *device , uint8_t endpoint_in, uint8_t endpoint_out)
{
    ...
    printk(KERN_INFO "Reset mass storage device");
	r1 = usb_control_msg(device,usb_sndctrlpipe(device,0),BOMS_RESET,BOMS_RESETREQTYPE,0,0,NULL,0,1000);
	if(r1<0)
		printk(KERN_INFO "error code: %d",r1);
	else
		printk(KERN_INFO "successful Reset");

	printk(KERN_INFO "Reading Max LUN: %d\n",*lun);
	r = usb_control_msg(device,usb_sndctrlpipe(device,0), BOMS_GET_MAX_LUN ,BOMS_GETMAXLUNREQTYPE,
	   0, 0, (void*)lun, 1, 1000);
	...
	printk(KERN_INFO "max_lba: %x,block_size found: %x,Device size found : %ld MB \n",max_lba,block_size ,device_size);
...
{   struct usb_device *device;
	...
	{
		printk(KERN_INFO "“USB drive detected is known”\n");
	}
...
int device_init(void)
{
	...
	printk(KERN_INFO "driver register");
	return 0;
}
